...
CREATE TABLE IF NOT EXISTS public.social_monthly_metrics (
  ...
  whatsapp_clicks         INTEGER CHECK (whatsapp_clicks IS NULL OR whatsapp_clicks >= 0),
  link_clicks             INTEGER CHECK (link_clicks IS NULL OR link_clicks >= 0),
  new_customers_reported  INTEGER CHECK (new_customers_reported IS NULL OR new_customers_reported >= 0),
  returning_customers_reported INTEGER CHECK (returning_customers_reported IS NULL OR returning_customers_reported >= 0),
  store_visits_reported   INTEGER CHECK (store_visits_reported IS NULL OR store_visits_reported >= 0),
