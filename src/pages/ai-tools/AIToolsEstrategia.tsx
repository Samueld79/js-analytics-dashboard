...
type Mode = 'formulario' | 'imagen' | 'transcripcion';

const MODES: { key: Mode; label: string }[] = [
  { key: 'formulario', label: '📋 Formulario paso a paso' },
...
function CampaignBlock({
  ...
  function updateAdSet<K extends keyof AdSet>(setId: string, field: K, value: AdSet[K]) {
    onChange(
      'conjuntos',
      campaign.conjuntos.map((s) => (s.id === setId ? { ...s, [field]: value } : s)),
    );
  }

  function addAdSet() {
    onChange('conjuntos', [...campaign.conjuntos, createEmptyAdSet()]);
  }

  function removeAdSet(setId: string) {
    onChange(
      'conjuntos',
      campaign.conjuntos.filter((s) => s.id !== setId),
    );
  }
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <input
            ...
            type="text"
            placeholder="Ej: OpticaPL_TOFU_Junio2026"
            ...
            <div style={{ display: 'flex', gap: 16, paddingTop: 6 }}>
              {(['ABO', 'CBO'] as const).map((v) => (
                <label key={v} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: '0.82rem', color: 'var(--color-text-secondary)' }}>
                  ...
                  <span>
                    <strong>{v}</strong> — {v === 'ABO' ? 'Por conjunto' : 'Advantage+'}
                  </span>
                </label>
              ))}
            </div>
        ...
        <div style={{ maxWidth: 240 }}>
          <label style={S.label}>
            {campaign.tipoPresupuesto === 'CBO'
              ...
              : 'Presupuesto por conjunto (COP/día)'}
          </label>
          {campaign.tipoPresupuesto === 'CBO' ? (
            <input
              className="form-input"
        ...
        <div>
          <p style={{
            ...
            color: 'var(--color-text-muted)', textTransform: 'uppercase', marginBottom: 4,
          }}>
          ...
          {campaign.conjuntos.map((s, si) => (
            <AdSetForm
              ...
              onRemove={() => removeAdSet(s.id)}
              canRemove={campaign.conjuntos.length > 1}
            />
          ))}
          <button
            className="btn-ghost"
...
export function AIToolsEstrategia() {
  ...
  function addCampaign() {
    setCampaigns((prev) => [...prev, { ...createEmptyCampaign(), id: uid() }]);
  }
  ...
  }, [mode, campaigns, kit, knowledge, contextNote, transcripcion, imageFile]);

  const formSummary: Record<string, string> = {
    mode,
    campaigns: campaigns.map((c) => c.nombre).join(', '),
    client: kit?.name ?? selectedClientId ?? '',
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 820 }}>
      ...
      {/* Mode tabs */}
      <div style={{
        display: 'flex', gap: 4, background: 'var(--color-bg-secondary)',
        padding: 4, borderRadius: 12, width: 'fit-content', flexWrap: 'wrap',
      }}>
        {MODES.map(({ key, label }) => (
          <button
            key={key}
            ...
            style={{
              ...
              background: mode === key ? 'var(--color-accent-cyan)' : 'transparent',
              color: mode === key ? 'var(--color-accent-cyan-fg, #000)' : 'var(--color-text-secondary)',
            }}
          >
            {label}
          </button>
      ...
      {mode === 'imagen' && (
        <div style={{ ...S.sectionCard, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <label style={S.label}>Imagen del mapa mental o pizarrón de estrategia <span style={{ color: '#ef4444' }}>*</span></label>
            {!imagePreview ? (
              <button
                type="button"
                ...
                style={{
                  ...
                  border: '2px dashed var(--color-border-strong)', borderRadius: 10,
              ...
              >
                <Image size={20} />
                Haz clic para subir la imagen (PNG, JPG, WEBP)
              </button>
            ) : (
          ...
          <div>
            ...
            <textarea
              className="form-textarea"
              rows={3}
              placeholder="¿Qué cliente es? ¿Hay algo específico que quieras que analice o estructure?"
              value={contextNote}
              onChange={(e) => setContextNote(e.target.value)}
            />
          </div>
        ...
        <div style={{ ...S.sectionCard, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <textarea
              className="form-textarea"
              rows={12}
              placeholder="Pega aquí la transcripción de voz o notas de reunión. Ej: 'Para Óptica vamos a montar una campaña de tráfico con ABO, dos conjuntos: el primero dirigido a Medellín mujeres 25-45, con interacción de IG los últimos 365 días...'"
              value={transcripcion}
