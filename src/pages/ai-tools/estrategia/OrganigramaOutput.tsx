...
function _buildJsonPrintHtml(data: OrganigramaData, dateStr: string): string {
  ...
  const campaignsHtml = (data.campanas ?? []).map((c, ci) => {
    const adSetsHtml = (c.conjuntos ?? []).map((s, si) => {
      const adsHtml = (s.anuncios ?? []).map((a, ai) => `
        <div class="ad">
          <div class="ad-hdr">🎨 ANUNCIO ${ai + 1} · ${esc(a.formato ?? '')}${a.angulo ? ` · <em>${esc(a.angulo)}</em>` : ''}</div>
          ${a.copy ? `<div class="copy">${esc(a.copy).replace(/\n/g, '<br>')}</div>` : ''}
          ${a.conversacion?.saludo ? `<div class="conv"><strong>Saludo:</strong> ${esc(a.conversacion.saludo)}${(a.conversacion.preguntas ?? []).map((p, pi) => `<br>P${pi + 1}: ${esc(p)}`).join('')}</div>` : ''}
        </div>`).join('');

      const aud = s.audiencia;
      const audRows = [
        aud?.lugares ? `<tr><td>Lugares</td><td>${esc(aud.lugares)}</td></tr>` : '',
        aud?.edad ? `<tr><td>Edad</td><td>${esc(aud.edad)}</td></tr>` : '',
        aud?.genero ? `<tr><td>Género</td><td>${esc(aud.genero)}</td></tr>` : '',
        aud?.incluir?.length ? `<tr><td>Incluir</td><td>${aud.incluir.map(esc).join(', ')}</td></tr>` : '',
        aud?.excluir ? `<tr><td>Excluir</td><td>${esc(aud.excluir)}</td></tr>` : '',
      ].filter(Boolean).join('');

      const plat = s.plataformas;
      const platHtml = plat?.activas?.length
        ? `<div class="plat">✅ ${plat.activas.map(esc).join(' · ')}${plat.excluidas?.length ? `<br>❌ ${plat.excluidas.map(esc).join(' · ')}` : ''}</div>`
        : '';

      return `<div class="adset">
        <div class="adset-hdr">📦 CONJUNTO ${si + 1}: ${esc(s.nombre ?? '')}</div>
        <div class="adset-meta">🎯 ${esc(s.objetivoRendimiento ?? '')}${s.ubicacionConversion ? ` · 📍 ${esc(s.ubicacionConversion)}` : ''}${s.presupuesto ? ` · 💰 ${esc(s.presupuesto)}` : ''}</div>
        ${audRows ? `<table class="aud">${audRows}</table>` : ''}
        ${platHtml}
        ${adsHtml}
      </div>`;
    }).join('');

    return `${ci > 0 ? '<div class="pb"></div>' : ''}
    <div class="campaign">
      <div class="camp-hdr">📋 CAMPAÑA ${ci + 1}: ${esc(c.nombre ?? '')}</div>
      <div class="camp-meta">
        <span class="badge">${esc(c.objetivo ?? '')}</span>
        <span class="badge">${esc(c.tipoPresupuesto ?? '')}</span>
        ${c.presupuesto ? `<span class="presup">💰 ${esc(c.presupuesto)}</span>` : ''}
      </div>
      ${c.razonamiento ? `<div class="razon">💡 ${esc(c.razonamiento)}</div>` : ''}
      ${adSetsHtml}
    </div>`;
  }).join('');

  const res = data.resumen;
  const resHtml = res ? `<div class="pb"></div>
  <div class="campaign">
    <div class="camp-hdr">📊 RESUMEN EJECUTIVO</div>
    <div class="metrics">
      ${res.totalDiario ? `<div class="metric"><span class="mv">${esc(res.totalDiario)}</span><span class="ml">Inversión diaria</span></div>` : ''}
      ${res.totalMensual ? `<div class="metric"><span class="mv">${esc(res.totalMensual)}</span><span class="ml">Inversión mensual</span></div>` : ''}
      <div class="metric"><span class="mv">${res.numCampanas ?? 0}</span><span class="ml">Campañas</span></div>
      <div class="metric"><span class="mv">${res.numConjuntos ?? 0}</span><span class="ml">Conjuntos</span></div>
      <div class="metric"><span class="mv">${res.numAnuncios ?? 0}</span><span class="ml">Anuncios</span></div>
    </div>
    ${res.kpis?.length ? `<div class="sec"><strong>KPIs objetivo</strong><ul>${res.kpis.map(k => `<li>${esc(k)}</li>`).join('')}</ul></div>` : ''}
    ${res.recomendaciones?.length ? `<div class="sec"><strong>Recomendaciones</strong><ol>${res.recomendaciones.map(r => `<li>${esc(r)}</li>`).join('')}</ol></div>` : ''}
  </div>` : '';

  const confirmarHtml = data.paraConfirmar?.length ? `<div class="campaign">
    <div class="camp-hdr">⚠️ Por confirmar con el cliente</div>
    <ul>${data.paraConfirmar.map(p => `<li>${esc(p)}</li>`).join('')}</ul>
  </div>` : '';

  return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
<title>Estrategia Meta Ads · Growth Strategy JS</title>
<style>
  @page{size:letter;margin:1.5cm}
  *{box-sizing:border-box}
  body{font-family:Arial,Helvetica,sans-serif;font-size:10pt;line-height:1.45;color:#111;background:#fff}
  .ph{display:flex;justify-content:space-between;align-items:baseline;border-bottom:2pt solid #000;padding-bottom:6pt;margin-bottom:14pt;font-size:9pt}
  .pf{position:fixed;bottom:0;left:0;right:0;text-align:center;font-size:7pt;color:#888;border-top:.5pt solid #ccc;padding:4pt;background:#fff}
  .pb{page-break-before:always;break-before:page;height:0;margin:0}
  .campaign{margin-bottom:14pt}
  .camp-hdr{font-size:11pt;font-weight:bold;background:#f0f0f0;padding:5pt 8pt;border-left:3pt solid #000;margin-bottom:6pt}
  .camp-meta{margin-bottom:4pt;font-size:9pt}
  .badge{display:inline-block;border:1pt solid #000;padding:1pt 6pt;font-size:8pt;font-weight:bold;margin-right:4pt}
  .presup{font-size:9pt;color:#333}
  .razon{font-style:italic;font-size:9pt;color:#444;margin-bottom:8pt}
  .adset{margin:8pt 0 8pt 12pt;border-left:2pt solid #555;padding-left:8pt}
  .adset-hdr{font-weight:bold;font-size:10pt;margin-bottom:3pt}
  .adset-meta{font-size:8.5pt;color:#333;margin-bottom:3pt}
  .aud{font-size:8.5pt;border-collapse:collapse;margin:4pt 0}
  .aud td{padding:1pt 8pt 1pt 0;vertical-align:top}
  .aud td:first-child{font-weight:bold;white-space:nowrap;color:#333}
  .plat{font-size:8.5pt;margin:3pt 0 5pt}
  .ad{margin:6pt 0 6pt 8pt;border-left:1.5pt solid #888;padding-left:8pt}
  .ad-hdr{font-weight:bold;font-size:9pt;margin-bottom:3pt}
  .copy{font-size:9.5pt;white-space:pre-wrap;background:#f8f8f8;border:.5pt solid #ddd;padding:6pt;margin:3pt 0;line-height:1.5}
  .conv{font-size:8.5pt;color:#444;margin-top:3pt}
  .metrics{display:flex;gap:10pt;flex-wrap:wrap;margin:6pt 0 8pt}
  .metric{text-align:center;border:1pt solid #ccc;padding:4pt 8pt;min-width:60pt}
  .mv{display:block;font-size:13pt;font-weight:bold}
  .ml{display:block;font-size:7pt;color:#555}
  .sec{margin:6pt 0;font-size:9.5pt}
  ul,ol{margin:3pt 0;padding-left:16pt}
  li{font-size:9pt;margin-bottom:2pt}
</style></head><body>
<div class="ph"><strong>Growth Strategy JS</strong><span>${esc(data.cliente ?? '')}${data.fecha ? ' · ' + esc(data.fecha) : ''}</span><span>${dateStr}</span></div>
<div class="pf">Documento confidencial · Growth Strategy JS</div>
${campaignsHtml}${resHtml}${confirmarHtml}
</body></html>`;
}
...
function _buildTextPrintHtml(output: string, dateStr: string): string {
  ...
  const htmlLines = output.split('\n').map((line) => {
    ...
    if (line.trim().startsWith('━━━')) {
      if (firstDivider) { firstDivider = false; return `<div class="divider">${esc}</div>`; }
      return `<div class="pb"></div><div class="divider">${esc}</div>`;
    }
    if (!line.trim()) return '<div class="el"></div>';
    return `<div>${esc}</div>`;
  }).join('');
...
<style>
  @page{size:letter;margin:1.5cm}
  *{box-sizing:border-box}
  body{font-family:'Courier New',Courier,monospace;font-size:10pt;line-height:1.55;color:#000;background:#fff;white-space:pre-wrap;word-break:break-word}
  .ph{display:flex;justify-content:space-between;align-items:baseline;border-bottom:1.5pt solid #333;padding-bottom:6pt;margin-bottom:14pt}
  .pf{position:fixed;bottom:0;left:0;right:0;text-align:center;font-size:7.5pt;color:#888;border-top:.5pt solid #ccc;padding:4pt;background:#fff}
  .pb{page-break-before:always;break-before:page;height:0}
  .el{height:5pt}
  .divider{color:#333}
</style></head><body>
<div class="ph"><strong>Growth Strategy JS</strong><span>${dateStr}</span></div>
<div class="pf">Documento confidencial · Growth Strategy JS</div>
${htmlLines}
</body></html>`;
}
